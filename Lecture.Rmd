...
`without_outliers` contains some 88,000 rows, meaning nearly 20,000 have been identified as outliers.
...
A further possibility is to use the function `facet_wrap` which creates single plot for the factors of a variable. Here, individual plots are displayed according to the labels ( = vowels) in the dataframe.
...
From the visualisation shown so far it becomes obvious that vowels are not neatly separated but rather often show a certain amount of overlap. This  is either due to measurement errors or is indicating that a vowel production is changing, i.e. that we are dealing with sound change. Sound change usually manifests itself in subconscious, minimal devitations from a former prototypical sound and these deviation may become more prominent at a later stage.

In Luxembourgish, too, several sound changes are in progress for the vowels. The last example in this tutorial covers the variational pattern of the two vowels [e] (in words like *Méck, bréngen, sécher*) and [ə] (like in *Schëff, Dësch*). While for elder speakers these vowels are distinct, there is an ongoing merger for younger speaker by pronuncing the vowel before the certain consonants like [ə] instead of [e].

To tackle this task, we first need the relevant data, which has been prepared in a dataframge `merger`.

```{r}
merger <- readRDS("e_merger_data.RDS")
```

With around 2,100 rows this dataset is not too large for a scatter plot. The relevant information is the horizontal overlap between the clouds. While two of the red ones are quite distinct from the green one, it is the could for [ɕ] which is showing overlap with the cloud for [ʃ]. Here we have clear evidence of an ongoing vowel merger.
...
Basically, high Pillai scores indicate distinct clouds, low Pillai score indicate merger.

```{r echo=FALSE}
knitr::include_graphics(rep("pillai_example.png"))
```

Let's not calculate the Pillai scores for the overlap.
